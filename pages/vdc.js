...
export default function VDCDemoPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Supports both sync and async buildLoginUrl implementations
        const url = await Promise.resolve(buildLoginUrl());
        if (mounted) setLoginUrl(String(url || ""));
      } catch {
        if (mounted) setLoginUrl("");
      }
    })();
        ...
        <main className="mainContent">
          <div className="container">
            <nav className="breadcrumb">
              ...
              <Link href="/life-sciences/evidence/index.html" className="breadcrumbLink">
                Architecture &amp; GxP Evidence
              </Link>
              <span className="sep">|</span>
              <Link href="/life-sciences/resources/index.html" className="breadcrumbLink">
            ...
            <section className="hero">
              ...
              <div className="infoBox">
                <p>
                  In regulated Life Sciences environments, teams must be able to show who performed an action, when it
                  occurred, under which role, and what controls enforced it.
                </p>
                <p>
                  This VDC demo is a working example using native AWS services. It demonstrates role-based access,
                  MFA-enforced approvals, controlled document access, immutable audit trails, and electronic signature
                  intent.
                </p>

                <div className="credentials">
                  <p>
                    <strong>How to use it:</strong> Sign in as Submitter:{" "}
                    <span className="cred">{SUBMITTER_EMAIL}</span>
                    <CopyIconButton value={SUBMITTER_EMAIL} ariaLabel="Copy email" /> Password:{" "}
                  ...
                  <p>
                    ...
                    <CopyIconButton value={APPROVER_EMAIL} ariaLabel="Copy email" /> Password:{" "}
                    <span className="cred">{PASSWORD}</span>
                    <CopyIconButton value={PASSWORD} ariaLabel="Copy password" />
                  </p>
            ...
            <section className="ctaSection">
              <div className="ctaContent">
                ...
                <p className="ctaText">
                  Open the demo, use the credentials above, run the Submitter to Approver flow, and sign out before
                  changing users.
                </p>
              </div>

              <a
                href={isReady ? loginUrl : "#"}
                onClick={(e) => {
                  if (!isReady) e.preventDefault();
                }}
               // remove target entirely
                rel="noopener noreferrer"
                className={`ctaButton ${!isReady ? "ctaButtonDisabled" : ""}`}
                aria-disabled={!isReady}
                title={!isReady ? "Preparing login link…" : "Open demo"}
              >
                {isReady ? "Go to the Demo →" : "Loading…"}
              </a>
            </section>
