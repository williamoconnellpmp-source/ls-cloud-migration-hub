...
export default function UploadPage() {
  ...
  // Clear â€œstuckâ€ errors when user changes relevant inputs
  useEffect(() => {
  ...
  async function initUpload() {
    setBusy(true);
    setError(null);
    setStatusMsg("Requesting secure upload URLâ€¦");
    ...
    try {
      ...
      setStatusMsg("Uploading file to controlled storageâ€¦");
      ...
      setStatusMsg("File uploaded. Submitting documentâ€¦");
      ...
      <form onSubmit={onClickSubmit} style={{ display: "grid", gap: "1rem", maxWidth: 900 }}>
        <section style={{ padding: "1rem", border: "1px solid #ddd" }}>
          ...
          <div style={{ marginTop: "0.5rem", color: "#444" }}>
            Selected: <strong>{file?.name || "â€”"}</strong>
          </div>
        </section>
